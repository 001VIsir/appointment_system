...
│                                                              │
│  ┌─────────────────────────────────────────────────────────┐  │
│  │  Nacos Config (配置中心 + 动态刷新 + @RefreshScope)    │  │
│  └─────────────────────────────────────────────────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
│  ┌─────────────────────────────────────────────────────────┐ │
│  │  配置管理                                                │ │
│  │  - appointment_system.properties (dev)                  │ │
│  │  - appointment_system.properties (test)                 │ │
│  │  - appointment_system.properties (prod)                 │ │
│  └─────────────────────────────────────────────────────────┘ │
│         │                    │                    │          │
│         ▼                    ▼                    ▼          │
│  ┌───────────┐       ┌───────────┐       ┌───────────┐     │
...
**配置加载顺序：**
1. bootstrap.properties（引导配置，Nacos 地址等）
2. Nacos 远程配置（可选，默认禁用）
3. application.properties（本地兜底）

**动态刷新原理：**
```
1. Nacos 控制台修改配置并发布
2. Nacos Server 通过 HTTP 长轮询通知客户端
3. Spring Cloud 接收到通知
4. 刷新 @RefreshScope Bean 的配置值
5. 下一次请求使用新配置
```
